...
int32 main()
{
	...
	if (data_table_ptr)
	{
		...
		std::cout << "EnumType:" << data_table_ptr->EnumType << std::endl;
		...
		std::cout << "VectorEnumType:" << std::endl;
		std::cout << "(";
		for (int32 i = 0; i < data_table_ptr->EnumTypeList.size(); i++)
		{
			if (i > 0)
			{
				std::cout << ",";
			}
			std::cout << data_table_ptr->EnumTypeList[i];
		}
		std::cout << ")" << std::endl;
	}
